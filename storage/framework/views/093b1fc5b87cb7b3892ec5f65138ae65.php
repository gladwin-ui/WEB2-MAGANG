<?php $__env->startSection('title', 'Progres Import #' . $importJob->id); ?>
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 mb-6 border-b border-border-strong">
        <div class="flex items-center gap-3.5">
            <a href="<?php echo e(route('import.upload')); ?>" class="w-11 h-11 rounded-xl bg-bg-secondary border border-border-default flex items-center justify-center shrink-0 shadow-sm text-text-secondary hover:text-accent hover:border-accent transition-all" title="Kembali">
            ...
            <div>
                ...
                <p class="text-xs md:text-sm text-text-secondary mt-0.5 font-medium font-mono">Job #<?php echo e($importJob->id); ?> &middot; <?php echo e($importJob->filename); ?></p>
            </div>
        </div>
    </div>
    ...
    <?php if(session('parse_warnings') && count(session('parse_warnings')) > 0): ?>
        <div class="mb-5 p-4 rounded-xl bg-yellow-50 border border-yellow-200 text-yellow-700 text-sm">
            ...
            <ul class="list-disc list-inside text-xs space-y-0.5">
                <?php $__currentLoopData = session('parse_warnings'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $warn): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li><?php echo e($warn); ?></li>
    ...
    <div id="progress-card" class="bg-white border border-slate-200 rounded-xl shadow-card p-6 mb-5">
        ...
        <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-2">
                ...
                <span id="status-label" class="text-sm font-bold text-slate-700 capitalize"><?php echo e($importJob->status); ?></span>
            </div>
        ...
        <div class="flex justify-between text-xs text-slate-400 mb-6">
            ...
            <span id="total-label">dari <?php echo e($importJob->total_rows); ?> baris</span>
        </div>
    ...
    <div id="action-buttons" class="hidden flex gap-3">
        <a href="<?php echo e(route('import.upload')); ?>" class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-200 text-slate-600 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors">
            <i class="bi bi-cloud-upload"></i> Import Lagi
        </a>
        <a href="<?php echo e(route('dashboard')); ?>" class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-colors"
...
<script>
    const JOB_ID      = <?php echo e($importJob->id); ?>;
    const STATUS_URL  = '<?php echo e(route('import.status', $importJob->id)); ?>';
    const TOTAL_ROWS  = <?php echo e($importJob->total_rows); ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\MAGANG\WEB2-MAGANG\resources\views/import/progress.blade.php ENDPATH**/ ?>